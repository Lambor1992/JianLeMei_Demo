...
import com.jlm.app.jianlemei_demo.activity.adapter.BMIAdapter;
...
public class BMIActivity extends Activity {
    ...
    private void initView() {
        Title.setTitleName("身体指数监控");
        NameTv.setText("身体指数");
        BMIAdapter adapter = new BMIAdapter(this);
        mListview.setAdapter(adapter);
        ExamSQLHelper examSQLHelper = new ExamSQLHelper(this);
        try {
            ArrayList<BMIBean> xList = (ArrayList<BMIBean>) examSQLHelper.getDao(BMIBean.class).queryForAll();
    ...
    private void initChartView(ArrayList<BMIBean> list) {
        ...
        for (int i = 0; i < numColumns; i++) {
            ...
            values.add(new SubcolumnValue(list.get(i).getBMI(), Color.parseColor("#f237e3")));
