...
import com.jlm.app.jianlemei_demo.activity.adapter.VitalCapAdapter;
...
public class VitalCapacityActivity extends Activity {
    ...
    private void initView() {
        Title.setTitleName("肺活量检测");
        NameTv.setText("肺活量");
        VitalCapAdapter adapter = new VitalCapAdapter(this);
        mListview.setAdapter(adapter);
        ExamSQLHelper examSQLHelper = new ExamSQLHelper(this);
        try {
            ArrayList<VitalCapacityBean> xList = (ArrayList<VitalCapacityBean>) examSQLHelper.getDao(VitalCapacityBean.class).queryForAll();
    ...
    private void initChartView(ArrayList<VitalCapacityBean> list) {
        ...
        for (int i = 0; i < numColumns; i++) {
            ...
            values.add(new SubcolumnValue(list.get(i).getVitalCapacity(), Color.parseColor("#f237e3")));
