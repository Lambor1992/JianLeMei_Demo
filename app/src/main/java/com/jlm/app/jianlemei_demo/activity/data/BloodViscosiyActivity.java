...
public class BloodViscosiyActivity extends Activity {
    ...
    private void initView() {
        Title.setTitleName("血液粘度监测");
        NameTv.setText("血液粘度");
        BloodViscosityAdapter adapter = new BloodViscosityAdapter(this);
        mListview.setAdapter(adapter);
        ExamSQLHelper examSQLHelper = new ExamSQLHelper(this);
        try {
            ArrayList<BloodViscosityBean> xList = (ArrayList<BloodViscosityBean>) examSQLHelper.getDao(BloodViscosityBean.class).queryForAll();
    ...
    private void initChartView(ArrayList<BloodViscosityBean> list) {
        ...
        for (int i = 0; i < numColumns; i++) {
            ...
            values.add(new SubcolumnValue(list.get(i).getBloodViscosity(), Color.parseColor("#f237e3")));
